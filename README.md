...
- 決定した仕様・運用・対策は [DECISIONS.md](DECISIONS.md) に日付付きで追記していきます
...
- `GET /videos/fc2` は成人向け（18+確認後に閲覧）です
- 取得方式は「API優先 → 未設定ならHTML抽出（スクレイピングfallback）」です
- 設定値は `.env.example` の FC2 セクションを参照してください
...
## FC2動画キャッシュの定期更新（GitHub Actions）

`/videos/fc2` の表示を安定させるため、GitHub Actions で `scripts/fetch_fc2_videos.js` を定期実行して MongoDB にキャッシュを書き込みます。

- ワークフロー: `.github/workflows/fetch-fc2-videos.yml`（1時間おき）
- 必須Secret: `MONGODB_URI`
- 任意Secret: `FC2_FETCH_LIMIT`（未設定ならデフォルトで25件）
