...
function getClientIp(req) {
  ...
  try {
    return String(req.ip || req.socket?.remoteAddress || 'unknown-ip').trim() || 'unknown-ip';
  } catch (_) {
    return 'unknown-ip';
  }
}
...
function createTrafficMonitor(options = {}) {
  ...
  const inboundUserAgents = new Map();
  const inboundIps = new Map();
  ...
  const flush = () => {
    ...
    const topAgents = Array.from(inboundUserAgents.entries())
      .sort((a, b) => b[1] - a[1])
      .slice(0, 8)
      .map(([key, count]) => `${key} count=${count}`);
    const topIps = Array.from(inboundIps.entries())
      .sort((a, b) => b[1] - a[1])
      .slice(0, 8)
      .map(([key, count]) => `${key} count=${count}`);
    ...
    if (topAgents.length > 0) logger(`[traffic] inbound-ua ${topAgents.join(' | ')}`);
    if (topIps.length > 0) logger(`[traffic] inbound-ip ${topIps.join(' | ')}`);

    inbound.clear();
    outbound.clear();
    inboundUserAgents.clear();
    inboundIps.clear();
  };
  ...
  return {
    recordInbound(req, statusCode, bytes) {
      ...
      const userAgent = normalizeUserAgent(req && req.headers ? req.headers['user-agent'] : '');
      ...
      inboundUserAgents.set(userAgent, (inboundUserAgents.get(userAgent) || 0) + 1);
      inboundIps.set(clientIp, (inboundIps.get(clientIp) || 0) + 1);
    },
