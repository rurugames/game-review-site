/**
 * GALLERY_ENABLE_OPENAI_ANALYSIS が有効なときだけ OpenAI Vision API で画像を判定する。
 * 既定では AI 判定を使わず normal を返す。
 *
...
function isOpenAIAnalysisEnabled() {
  const flag = (process.env.GALLERY_ENABLE_OPENAI_ANALYSIS || '').trim().toLowerCase();
  return flag === '1' || flag === 'true' || flag === 'yes' || flag === 'on';
}

function canUseOpenAIAnalysis() {
  return isOpenAIAnalysisEnabled() && !!process.env.OPENAI_API_KEY;
}
...
module.exports = { analyzeImage, buildMeta, initCounters, mimeFromExt, sleep, canUseOpenAIAnalysis, isOpenAIAnalysisEnabled };
