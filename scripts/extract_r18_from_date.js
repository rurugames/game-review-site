...
async function main() {
  ...
  if (!ym || !/^\d{4}-\d{2}$/.test(ym)) {
    throw new Error('Usage: node scripts/extract_r18_from_date.js YYYY-MM YYYY-MM-DD');
  }
  ...
  const games = await dlsite.fetchGamesByMonth(year, month);

  const fetchedPath = path.join('csvoutput', `fetched_games_${ym}.json`);
  if (fs.existsSync(fetchedPath)) {
    const backup = nextPath(path.join('csvoutput', `fetched_games_${ym}_old.json`));
    fs.copyFileSync(fetchedPath, backup);
    console.log('BACKUP fetched ->', backup);
  }
  fs.writeFileSync(fetchedPath, JSON.stringify(games, null, 2), 'utf8');
  console.log('WROTE fetched ->', fetchedPath, 'count=', games.length);
  ...
  console.log('WROTE', outJsonMissing, 'count=', missing.length);
